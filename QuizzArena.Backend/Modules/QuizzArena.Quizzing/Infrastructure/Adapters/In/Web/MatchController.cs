...
) : ControllerBase
{
    ...
    public async Task<ActionResult<MatchDetailResponseDto>> GetMatchById(Guid matchId)
    {
        MatchDetailResponseDto response = await getMatchByIdUseCase.Execute(matchId);
        return Ok(response);
    }
