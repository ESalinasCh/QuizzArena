...
{
    public async Task<List<GetClassSourceResponseDto>> Execute(Guid userId)
    {
        List<(ClassSource Source, List<Guid> ProcessingJobsIds)> results = await classSourceRepository.GetByUserIdAsync(userId);
